...
Core.prototype.inputHandler = function (e) {
    ...
    if (input.length > 0) {
        ...
        if (inputWords.length === 1 && 'enable'.indexOf(inputWords[0].toLowerCase()) === 0) {
            Object.keys(PackageManager.registeredPackages).forEach(function (name) {
                if (typeof PackageManager.enabledPackages[name] === 'undefined') {
                    commands.push(new EnablePackage(name));
                }
            });
        } else if (inputWords.length > 1 && 'enable'.indexOf(inputWords[0].toLowerCase()) === 0) {
            Object.keys(PackageManager.registeredPackages).forEach(function (name) {
                if (name.toLowerCase().indexOf(inputWords[1].toLowerCase()) > -1 && typeof PackageManager.enabledPackages[name] === 'undefined') {
                    commands.push(new EnablePackage(name));
                }
            });
        }
...
EnablePackage.prototype.run = function () {
    PackageManager.enable(this.packageName);
};
...
DisablePackage.prototype.run = function () {
    PackageManager.disable(this.packageName);
};
